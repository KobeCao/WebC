...
class Tab {
    ...
    init() { // 页面一加载，给所有的元素绑定事件
        ...
        this.add.onclick = this.addTab; //点击按钮不需要循环。添加操作
        for(var i = 0; i < this.lis.length;i++) {
            this.lis[i].index = i; // 给每一个小li都添加一个属性index,这个属性里面存着我当前小li里面的索引号
            this.lis[i].onclick = this.toggleTab; // 点击完之后再调用，不加小括号
            // 给每一个删除按钮都要绑定一个点击事件
            this.remove[i].onclick = this.removeTab; //调用函数之后，拿到当前的索引号。当前remove[i]没有索引号，但是它的爸爸有
